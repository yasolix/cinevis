...
def predict(X_img_path, knn_clf = None, model_save_path ="", DIST_THRESH = .5):
    ...
    return [(pred, loc, encod) if rec else ("N/A", loc,encod) for pred, loc, rec, encod in
            zip(knn_clf.predict(faces_encodings), X_faces_loc, is_recognized, faces_encodings)]
...
def draw_preds(img_path, preds):
    ...
    #source_img.save("/tmp/"+timestr+".png","PNG")

import glob

def _pred_to_str(pred):

    loc = pred[1]
    name = pred[0]
    # (top, left, bottom, right)
    locstr = ' '.join(",%d" % l for l in [loc[0], loc[3], loc[2], loc[1]])
    encstr = ' '.join(",%g" % l for l in pred[2])

    return name + locstr + encstr
...
if __name__ == "__main__":
    ...
    '''
    frames_folder = "/home/yt/Desktop/cvpr2014/repro/mediaeval/data/dataset/movframes/"
    faces_folder = "/home/yt/Desktop/cvpr2014/repro/mediaeval/data/dataset/facepreds/"
    frame_no = 1
    filename = faces_folder + "Chatter-faces" + ".csv"
    ff = open(filename, "w")

    for img in sorted(glob.glob(frames_folder+'Chatter*')):
        print(img)

        preds = predict(img, knn_clf=knn_clf)
        #print(preds)
        if len(preds):
            #draw_preds(img,preds)
            ##ff.write(str(preds))
            for p in preds:
                line = str(frame_no) + ',' + _pred_to_str(p) +'\n'
                ff.write(line)
